...
getCamera :: SF a ToScreen
getCamera = global >>> arr (\g ->
  mkToScreen $ fromMaybe 0 $ os_pos =<< M.lookup Camera (g_everyone g))
...
newtype ToScreen = ToScreen
  { toScreen :: V3 Double -> (V2 Double, Double, Double)
  }


mkToScreen :: V3 Double -> ToScreen
mkToScreen camPos = ToScreen $ \(V3 wx wy wz) -> do
    let V4 sx sy _ sw = projection !* (m !* V4 (-wx) wy wz 1)
...
camMat :: M44 Double
camMat = lookAt camEye (V3 0 0 0) $ V3 0 0 1
...
toScreenNormalized :: ToScreen -> V3 Double -> V2 Double
toScreenNormalized scr
